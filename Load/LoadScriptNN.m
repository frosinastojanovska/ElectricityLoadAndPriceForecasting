...
% The dataset is divided into two sets, a _training_ and a _test_ set. The 
% training set is used for building the model (estimating its parameters).
% The test set is used only for forecasting to test the performance of the
% model on out-of-sample data. 

% Create training set
size = length(data); 
rand_indices = randperm(size);
trainingPercent = round(size * 0.8);    %calculating 80%
